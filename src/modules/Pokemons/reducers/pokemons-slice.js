...
export const getAllPokemons = createAsyncThunk('pokemons/getAll', async () => {
  const response = await fetch('https://pokeapi.co/api/v2/pokemon?limit=151')
  const data = await response.json()
  return data.results
})

export const getPokemonDetails = createAsyncThunk('pokemons/getPokemonDetails', async (url) => {
